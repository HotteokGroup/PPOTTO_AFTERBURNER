...
export const imageProcess: Handler = async (event: S3Event) => {
  ...
  const fileName = originalKey.match(/(?<=\/)[^\/]+(?=\.\w+)/);
  if (!fileName) {
    console.log(JSON.stringify(event));
  ...
  const [smallImageSaveResult, largeImageSaveResult, originalCompressImageSaveResult] = await Promise.allSettled([
    saveFileToS3({ bucket: bucketName, key: `output/${TODAY}/${fileName}/small.webp`, file: smallImage.value }),
    saveFileToS3({ bucket: bucketName, key: `output/${TODAY}/${fileName}/large.webp`, file: largeImage.value }),
    saveFileToS3({
      bucket: bucketName,
      key: `output/${TODAY}/${fileName}/original-compress.webp`,
      file: originalCompressImage.value,
    }),
  ]);
  if (
